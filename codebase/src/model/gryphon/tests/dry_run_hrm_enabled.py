...
def main():
    # Create a minimal config with hrm_enabled=True and small sizes
    config = get_gryphon_small_config()
    # Align HRM seq_len with input length to avoid shape mismatch
    config.max_sequence_length = 256  # must be divisible by block_size and > num_global_blocks
    config.block_size = 64
    config.num_global_blocks = 2
    config.window_size = 3
    config.num_random_blocks = 2
    config.hrm_enabled = True
    config.hrm_use_act = False  # keep HRM simple for dry-run
